...
const getAllProperties = async (req, res) => {
    try {
        ...
        if (amenity && petFriendly === 'true') {
            filter.amenities = { $all: [
                { $elemMatch: { $regex: amenity, $options: 'i' } },
                { $elemMatch: { $regex: 'pet', $options: 'i' } },
            ] };
        } else if (amenity) {
            filter.amenities = { $regex: amenity, $options: 'i' };
        } else if (petFriendly === 'true') {
            filter.amenities = { $regex: 'pet', $options: 'i' };
        }
        if (minRent || maxRent) {
            filter.rent = {};
