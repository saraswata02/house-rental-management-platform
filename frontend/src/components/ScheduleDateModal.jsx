...
function ScheduleDateModal({ isOpen, onClose, visit, onRescheduleRequested }) {
  ...
  const handleSend = async () => {
    try {
      setLoading(true);
      setError("");
      const customNote = note.trim() || `Owner is not available on ${visit.visitDate}. Please choose another date.`;
    ...
    <div className="modal-overlay">
      <div className="booking-modal" style={{ maxWidth: "480px" }}>
        ...
        <p style={{ color: "#64748b", fontSize: "14px", marginTop: "-6px", marginBottom: "16px" }}>
          Mark <strong>{visit.visitDate}</strong> as unavailable for <strong>{visit.tenant?.firstName} {visit.tenant?.lastName}</strong> and request them to pick an alternate date.
        </p>
        ...
        <div className="form-group">
          ...
          <textarea
            rows="3"
            placeholder="e.g. Sorry, I am unavailable on this date. Please select another date from the available options."
            value={note}
